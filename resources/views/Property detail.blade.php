...
.back-listing-wrap{margin-bottom:22px}
.back-listing-btn{display:inline-flex;align-items:center;gap:9px;background:#2d2926;color:#fff;padding:11px 20px;border-radius:10px;font-size:14px;font-weight:600;transition:.25s ease;box-shadow:0 4px 12px rgba(0,0,0,.08)}
.back-listing-btn:hover{background:#1a1816;transform:translateX(-3px);color:#fff}
.property-layout{display:grid;grid-template-columns:1fr 360px;gap:28px;align-items:start}
...
.prop-image .type-badge{position:absolute;top:16px;left:16px;background:#2d2926;color:#fff;padding:6px 16px;border-radius:20px;font-size:12px;font-weight:600;text-transform:capitalize;z-index:3}
.prop-image .status-badge{position:absolute;top:16px;right:16px;background:#e8f5e9;color:#2e7d32;padding:6px 16px;border-radius:20px;font-size:12px;font-weight:600;z-index:3}
...
.star-rating-input input:checked~label,.star-rating-input label:hover,.star-rating-input label:hover~label{color:#f5a623}
.rating-form-card textarea{width:100%;margin-top:14px;padding:10px 12px;border-radius:10px;border:1.5px solid #e0e0e0;font-family:inherit;font-size:14px;resize:vertical;background:#fafafa}
.rating-form-card textarea:focus{border-color:#2d2926;background:#fff;outline:none}
.rating-submit-btn{margin-top:14px;background:#2d2926;color:#fff;padding:11px 28px;border:none;border-radius:24px;font-size:14px;font-weight:700;cursor:pointer;transition:.2s}
...
.fgroup label{display:block;font-size:12px;font-weight:700;color:#555;margin-bottom:5px;text-transform:uppercase;letter-spacing:.4px}
.fgroup input,.fgroup select,.fgroup textarea{width:100%;padding:11px 14px;border:1.5px solid #e0e0e0;border-radius:10px;font-size:14px;color:#333;outline:none;transition:.2s;background:#fafafa;font-family:'Segoe UI',Arial,sans-serif}
.fgroup input:focus,.fgroup select:focus,.fgroup textarea:focus{border-color:#2d2926;background:#fff;box-shadow:0 0 0 3px rgba(45,41,38,.08)}
...
.book-btn{width:100%;padding:14px;border:none;border-radius:12px;background:#2d2926;color:#fff;font-size:16px;font-weight:700;cursor:pointer;transition:.2s;display:flex;align-items:center;justify-content:center;gap:8px}
.book-btn:hover{background:#1a1a1a;transform:translateY(-1px)}
.login-prompt{text-align:center;padding:20px;background:#f8f4f0;border-radius:12px;border:1px dashed #d4b896}
.login-prompt p{font-size:13px;color:#888;margin-bottom:12px}
.login-prompt a{display:inline-flex;align-items:center;gap:6px;background:#2d2926;color:#fff;padding:10px 24px;border-radius:20px;font-size:13px;font-weight:600;transition:.2s}
...
.carousel-arrow{position:absolute;top:50%;transform:translateY(-50%);width:42px;height:42px;background:rgba(255,255,255,.9);border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:15px;color:#2d2926;box-shadow:0 2px 10px rgba(0,0,0,.2);transition:.2s;z-index:5}
.carousel-arrow:hover{background:#fff;transform:translateY(-50%) scale(1.08)}
.carousel-prev{left:16px}
.carousel-next{right:16px}
.carousel-counter{position:absolute;bottom:16px;right:16px;background:rgba(0,0,0,.6);color:#fff;font-size:12px;padding:4px 12px;border-radius:20px;z-index:5}
.carousel-slide{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .4s ease;pointer-events:none}
.carousel-slide.active{opacity:1;pointer-events:auto}
.carousel-thumbs{display:flex;gap:8px;margin-bottom:20px;overflow-x:auto;padding-bottom:4px}
.carousel-thumbs img{width:76px;height:56px;object-fit:cover;border-radius:8px;cursor:pointer;opacity:.55;border:2px solid transparent;transition:.2s;flex-shrink:0}
.carousel-thumbs img:hover{opacity:.85}
.carousel-thumbs img.active{opacity:1;border-color:#2d2926}
.property-map-link{padding:20px 24px}
.property-map-link .section-title{display:flex;align-items:center;gap:8px}
.property-map-link .section-title i{color:#c8a882}
.map-location-link{display:inline-flex;align-items:center;gap:9px;color:#1877f2;font-size:14px;font-weight:600;text-decoration:none;transition:.2s ease}
.map-location-link:hover{color:#0d5dcc;text-decoration:underline}
.map-location-link i:first-child{font-size:16px}
.map-location-link i:last-child{font-size:11px}
.map-url-text{overflow-wrap:anywhere;word-break:break-word;line-height:1.6}
@media(max-width:768px){
.page-wrap{margin:20px auto;padding:0 15px 40px}
.property-layout{grid-template-columns:1fr}
.booking-card{position:static}
.prop-image{height:280px}
.dates-grid{grid-template-columns:1fr}
.features-row{gap:14px}
.prop-title{font-size:21px}
}
</style>
</head>
<body>
...
<a href="{{ route('home') }}" class="back-listing-btn">
...
foreach($property->images as $img){$allImages->push($img->image_path);}
@endphp

<div class="prop-image">
@if($allImages->count()>0)
@foreach($allImages as $index=>$imgPath)
<img src="{{ asset('storage/'.$imgPath) }}" class="carousel-slide {{ $index==0?'active':'' }}" alt="{{ $property->title }}">
@endforeach

@if($allImages->count()>1)

<div class="carousel-arrow carousel-prev" onclick="changeSlide(-1)"><i class="fa-solid fa-chevron-left"></i></div>
<div class="carousel-arrow carousel-next" onclick="changeSlide(1)"><i class="fa-solid fa-chevron-right"></i></div>
<div class="carousel-counter"><span id="current-slide">1</span> / {{ $allImages->count() }}</div>
@endif
@else
<i class="fa-solid fa-building no-img"></i>
@endif

<span class="type-badge">{{ ucfirst($property->type) }}</span> <span class="status-badge"><i class="fa-solid fa-circle-check"></i> Available</span>
...
@foreach($allImages as $index=>$imgPath)
<img src="{{ asset('storage/'.$imgPath) }}" class="thumb {{ $index==0?'active':'' }}" onclick="goToSlide({{ $index }})" alt="Property Image">
@endforeach
</div>
@endif
...
<div class="property-map-link">
<div class="section-title"><i class="fa-solid fa-location-dot"></i> Property Location</div>
<a href="{{ $property->map_link }}" target="_blank" rel="noopener noreferrer" class="map-location-link" title="{{ $property->map_link }}">
<i class="fa-solid fa-map-location-dot"></i>
<span class="map-url-text">{{ $property->map_link }}</span>
...
{{ number_format($property->ratings_avg_stars,1) }}
({{ $property->ratings_count }} {{ $property->ratings_count==1?'review':'reviews' }})
</span>
</div>
@else
...
<img src="{{ asset('storage/'.$property->user->profile_picture) }}" alt="{{ $property->user->name }}">
@else
...
@for($i=5;$i>=1;$i--)
<input type="radio" id="star{{ $i }}" name="stars" value="{{ $i }}" {{ old('stars',($userRating?->stars??0))==$i?'checked':'' }} required>
...
<div class="booking-price">₨ {{ number_format($property->price) }} <span>/ month</span></div>
...
<a href="{{ route('property.edit',$property->id) }}"><i class="fa-solid fa-pen-to-square"></i> Edit Listing</a>
</div>
@else
...
<input type="date" name="check_in" value="{{ old('check_in') }}" min="{{ date('Y-m-d') }}" required>
</div>
<div class="fgroup">
<label>Check Out</label>
<input type="date" name="check_out" value="{{ old('check_out') }}" min="{{ date('Y-m-d',strtotime('+1 day')) }}" required>
</div>
</div>

@error('check_in') <span class="error-msg">{{ $message }}</span>
@enderror
@error('check_out') <span class="error-msg">{{ $message }}</span>
@enderror
...
const inDate=new Date(checkIn.value),outDate=new Date(checkOut.value);
